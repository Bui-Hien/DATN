...
import com.buihien.datn.service.BankService;
...
@Service
public class BankServiceImpl extends GenericServiceImpl<Bank, BankDto, SearchDto> implements BankService {
    @Override
    protected BankDto convertToDto(Bank entity) {
        return new BankDto(entity);
    }

    @Override
    protected Bank convertToEntity(BankDto dto) {
        Bank entity = null;
        if (dto.getId() != null) {
    ...
    @Override
    public Page<BankDto> pagingSearch(SearchDto dto) {
        ...
        StringBuilder sqlCount = new StringBuilder("SELECT COUNT(entity.id) FROM Bank entity WHERE (1=1) ");
        StringBuilder sql = new StringBuilder("SELECT new com.buihien.datn.dto.BankDto(entity) FROM Bank entity WHERE (1=1) ");
