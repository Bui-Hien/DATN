...
import {
    ...
    deleteMultipleSalaryTemplateByIds,
...
import {SalaryTemplateObject} from "./SalaryTemplate";
...
export default class SalaryTemplateStore {
    ...
    selectedRow = new SalaryTemplateObject();
    selectedDataList = [];
    isOpenFilter = false;
    ...
    pagingSalaryTemplate = async () => {
        try {
            const response = await pagingSalaryTemplate(this.searchObject);
    ...
    setPageIndex = async (page) => {
        ...
        await this.pagingSalaryTemplate();
    };
    ...
    handleOpenCreateEdit = async (id) => {
        try {
            if (id) {
                const {data} = await getSalaryTemplateById(id);
                this.selectedRow = {
                    ...new SalaryTemplateObject(),
                    ...data.data,
                };
            } else {
                this.selectedRow = new SalaryTemplateObject();
            }
    ...
    handleConfirmDelete = async () => {
        try {
            const {data} = await deleteSalaryTemplate(this.selectedRow.id);
    ...
    handleConfirmDeleteMultiple = async () => {
        try {
            ...
            await deleteMultipleSalaryTemplateByIds(ids);
            this.selectedDataList = [];
            toast.success(i18n.t("toast.delete_success"));
            await this.pagingSalaryTemplate();
            this.handleClose();
